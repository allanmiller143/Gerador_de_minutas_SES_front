...
export function UploadDraftModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleUpload = () => {
    if (!file || !numero || !assunto || !prioridade) {
      toast("Por favor, preencha todos os campos e selecione um arquivo.");
      return;
    }
    ...
    uploadMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast("Rascunho criado com sucesso!");
          handleClose();
        },
        onError: (err: any) => {
          toast(err?.message || "Erro ao fazer upload do rascunho.");
          console.error(err);
        }
      }
    );
        ...
        <div className="grid gap-4 py-4 overflow-hidden">
          ...
          <div className="grid gap-2">
            ...
            <Input 
              ...
              placeholder="Ex: 12345.6789/2026-10" 
            />
          </div>
